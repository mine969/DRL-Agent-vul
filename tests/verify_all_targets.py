...
def verify_ecommerce():
    ...
    try:
        s.post(
            f"{url}/register",
            data={"username": "tester", "password": "pw", "email": "t@t.com"},
        )
        s.post(f"{url}/login", data={"username": "tester", "password": "pw"})
    except:
        pass
    return test_xss_generic(
        url,
        endpoint="/api/posts",
        param_name="content",
...
def verify_social():
    ...
    s.post(
        f"{url}/register",
        data={"username": "tester", "email": "t@t.com", "password": "pw"},
    )
    s.post(f"{url}/login", data={"username": "tester", "password": "pw"})
...
def verify_blog():
    ...
    return test_xss_generic(
        url, method="GET", endpoint="/", param_name="search", extra_data={}
    )
...
def verify_banking():
    ...
    return test_xss_generic(
        ...
        extra_data={"to_account": "1002", "amount": "1"},
        session=s,
...
def run_all():
    ...
    for name in TARGETS.keys():
        ...
        try:
            if name == "ecommerce":
                results[name] = verify_ecommerce()
            elif name == "social":
                results[name] = verify_social()
            elif name == "blog":
                results[name] = verify_blog()
            elif name == "banking":
                results[name] = verify_banking()
            elif name == "fileshare":
                results[name] = verify_fileshare()
        except Exception as e:
