...
def test_xss_detection():
    ...
    session.post(f"{url}/register", data={"username": "testuser", "password": "password123", "email": "test@example.com"})
    session.post(f"{url}/login", data={"username": "testuser", "password": "password123"})
    ...
    for payload in payloads:
        # Test Posts Endpoint
        try:
            r = session.post(f"{url}/api/posts", json={"title": "Test", "content": payload}, timeout=2)
