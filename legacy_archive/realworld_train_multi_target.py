...
class MultiTargetTrainer:
    ...
    def __init__(self, targets, model_path="dqn_web_sec_model.pth", verbose=False):
        ...
        try:
            ...
            self.agent.brain.load_state_dict(
        ...
        try:
            for episode in range(start_episode, total_episodes + 1):
                ...
                if self.verbose:
                    print(f"\n{'='*70}", flush=True)
                    print(
                        f"🎯 Episode {episode}/{total_episodes} | Target: {target_name}",
                        flush=True,
                    )
                ...
                reward, vulns_found = self._train_episode(
                    target_name, target_url, episode
                )
                ...
                if self.verbose:
                    print(
                        f"    Episode {episode} Complete | Reward: {reward:.1f} | Vulns: {vulns_found}"
                    )
            ...
            print(
                f"✅ Checkpoint saved: checkpoints/multi_target_ep{current_episode}.pth"
            )
            print(f"\n💡 To resume training, run:")
            print(
                f"   python train_multi_target.py --episodes {total_episodes} --resume {current_episode}"
            )
    ...
    def _train_episode(self, target_name, target_url, episode_num):
        ...
        while not done and steps < max_steps:
            ...
            if self.verbose:
                ...
                print(
                    f"    Step {steps}: Action={action_name} | Reward={reward:.1f} | Vuln={reward>50}",
                    flush=True,
                )
    ...
    def _print_progress(self, episode, total_episodes):
        ...
        for name, _ in self.targets:
            if self.episode_rewards[name]:
                ...
                print(
                    f"  {name:20} | Avg Reward: {avg_reward:6.1f} | Avg Vulns: {avg_vulns:.1f}"
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Multi-Target DRL Security Scanner Training"
    )
    parser.add_argument(
        "--episodes", type=int, default=1000, help="Total training episodes"
    )
    parser.add_argument(
        "--model", default="dqn_web_sec_model.pth", help="Model save path"
    )
    parser.add_argument(
        "--resume", type=int, default=0, help="Resume from episode number"
    )
    parser.add_argument(
        "--latest",
        action="store_true",
    ...
    if args.resume > 0:
        ...
        try:
            ...
            trainer.agent.brain.load_state_dict(
    ...
    trainer.train_curriculum(
        ...
        start_episode=args.resume + 1 if args.resume > 0 else 1,
    )
