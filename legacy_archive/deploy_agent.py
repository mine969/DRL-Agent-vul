...
def load_trained_agent(model_path, state_dim=11, action_dim=100):
    ...
    episode = load_model_smart(
        agent, model_path=model_path, auto_checkpoint=True, verbose=True
    )
...
def test_agent(
    ...
    for episode in range(episodes):
        ...
        while not done and step_count < 50:
            ...
            if reward > 50:  # Vulnerability found!
                ...
                episode_log.append(
                    f"  VULNERABILITY FOUND: {action_name} (Reward: {reward})"
                )
    ...
    if vulnerabilities_found:
        ...
        for vuln in vulnerabilities_found:
            print(
                f"  - Episode {vuln['episode']}: {vuln['action']} (Step {vuln['step']}, Reward: {vuln['reward']})"
            )
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Deploy trained RL agent against target websites"
    )
    parser.add_argument(
        "--target",
        type=str,
        required=True,
        help="Target URL (e.g., http://localhost/dvwa)",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="checkpoints/multi_target_10k_ep10000.pth",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Exploration rate (0.0 = exploitation only)",
    )
    parser.add_argument(
