...
def patch_file(filepath):
    ...
    sqli_login_pattern = (
        r"(jsonify\({\s*[^}]+?'message':\s*'Login successful'[^}]+?\}\))"
    )
    ...
    def sqli_login_sub(match):
        ...
        return (
            match.group(1)
            + "; response = make_response(response); response.headers['X-Vuln-Confirmed'] = 'sqli_login_bypass'"
        )
    ...
    idor_patterns = [
        (
            r"(return\s+jsonify\(order\))",
            "response = make_response(jsonify(order)); response.headers['X-Vuln-Confirmed'] = 'idor_access'; return response",
        ),
        (
            r"(return\s+jsonify\(user\))",
            "response = make_response(jsonify(user)); response.headers['X-Vuln-Confirmed'] = 'idor_access'; return response",
        ),
        (
            r"(return\s+jsonify\(profile\))",
            "response = make_response(jsonify(profile)); response.headers['X-Vuln-Confirmed'] = 'idor_access'; return response",
        ),
    ]
