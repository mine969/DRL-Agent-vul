...
class MultiTargetEvaluator:
    ...
    def __init__(self, model_path: str = "dqn_web_sec_model.pth"):
        ...
        episode = load_model_smart(
            self.agent, model_path=model_path, auto_checkpoint=True, verbose=True
        )
    ...
    def evaluate_target(
        self, target_name: str, target_url: str, num_episodes: int = 10
    ) -> Dict:
        ...
        for episode in range(1, num_episodes + 1):
            ...
            while not done and steps < max_steps:
                ...
                if reward > 50:
                    vulns_found += 1
                    action_name = (
                        env.action_book.get(action).__name__
                        if action in env.action_book
            ...
            print(
                f"  Episode {episode}/{num_episodes}: Reward={total_reward:.1f}, Vulns={vulns_found}, Steps={steps}"
            )
    ...
    def evaluate_all_targets(
        self, targets: List[Tuple[str, str]], num_episodes: int = 10
    ) -> Dict:
    ...
    def generate_report(
        self, results: Dict, output_file: str = "evaluation_report.json"
    ):
    ...
    def _print_summary(self, summary: Dict):
        ...
        print(
            f"Overall Avg Reward: {summary['overall_avg_reward']:.2f} ± {summary['overall_std_reward']:.2f}"
        )
        ...
        print(
            f"Worst Performing: {summary['worst_performing_target']['name']} ({summary['worst_performing_target']['avg_reward']:.2f})"
        )
        print("=" * 70)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", default="dqn_web_sec_model.pth", help="Path to trained model"
    )
    parser.add_argument("--episodes", type=int, default=10, help="Episodes per target")
    parser.add_argument(
        "--output", default="evaluation_report.json", help="Output report file"
    )
