...
def test_xss_endpoints():
    ...
    response = client.post(
        "/api/posts",
        json={"title": "Test Post", "content": "<script>alert(1)</script>"},
    )
    ...
    if (
        ...
        and response.headers.get("X-Vuln-Confirmed") == "xss_stored_posts_success"
    ):
    ...
    response = client.post(
        "/api/posts/1/comments", json={"content": "<script>alert(1)</script>"}
    )
    ...
    if (
        response.status_code == 200
        and response.headers.get("X-Vuln-Confirmed") == "xss_stored_comments_success"
    ):
