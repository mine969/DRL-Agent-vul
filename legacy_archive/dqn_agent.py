...
class ExperienceMemory:
    ...
    def recall_batch(
        ...
        if batch_size > self.current_size:
            raise ValueError(
                f"Batch size {batch_size} exceeds available experiences {self.current_size}"
            )
...
class NeuralNetworkBrain(nn.Module):
    ...
    def __init__(
        ...
        self.value_stream = nn.Sequential(
            nn.Linear(feature_output_size, 128), nn.ReLU(), nn.Linear(128, 1)
        )

        # Advantage stream: Estimates advantage of each action
        self.advantage_stream = nn.Sequential(
            nn.Linear(feature_output_size, 128), nn.ReLU(), nn.Linear(128, output_size)
        )
...
class DQNAgent:
    ...
    def __init__(
        self, state_dim: int, action_dim: int, config: Optional[AgentConfig] = None
    ):
        """
        ...
        self.brain = NeuralNetworkBrain(
            state_dim, action_dim, hidden_sizes=config.hidden_sizes
        ).to(self.device)
    ...
    def replay(self) -> Optional[float]:
        ...
        with torch.no_grad():
            ...
            next_q_values = (
                self.target_brain(next_states).gather(1, best_actions).squeeze(1)
            )
            # Bellman equation: Q* = r + gamma * max Q(s', a')
            target_q_values = (
                rewards.squeeze(1) + (1 - dones.squeeze(1)) * self.gamma * next_q_values
            )
    ...
    def soft_update(self) -> None:
        ...
        for target_param, main_param in zip(
            self.target_brain.parameters(), self.brain.parameters()
        ):
    ...
    def load(self, filepath: str, strict: bool = True) -> None:
        ...
        try:
            ...
            # Load network states
            self.brain.load_state_dict(checkpoint["brain_state_dict"], strict=strict)
            self.target_brain.load_state_dict(
                checkpoint["target_brain_state_dict"], strict=strict
            )
