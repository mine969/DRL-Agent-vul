...
class TargetHunter:
    ...
    def auto_generate_targets(self, source='all', max_per_source=3):
        ...
        if source in ['google', 'all']:
            ...
            dorks = random.sample(self.get_common_dorks(), min(max_per_source, len(self.get_common_dorks())))
            for dork in dorks:
                print(f"  → Trying: {dork}")
        ...
        if source in ['shodan', 'all'] and self.shodan_api_key:
            ...
            queries = random.sample(self.get_shodan_queries(), min(max_per_source, len(self.get_shodan_queries())))
        ...
        if source in ['crtsh', 'all']:
            ...
            domains = random.sample(self.get_target_domains(), min(max_per_source, len(self.get_target_domains())))
        ...
        if source in ['duckduckgo', 'all']:
            ...
            queries = random.sample(self.get_duckduckgo_queries(), min(max_per_source, len(self.get_duckduckgo_queries())))
            for query in queries:
                print(f"  → Trying: {query}")
