...
class VulnerabilityValidator:
    ...
    def _validate_sqli(self, response) -> bool:
        ...
        if response.status_code == 200 and (
            "SQL" in response.text or "Warning" in response.text
        ):
            return True
    ...
    def _validate_xss(self, response, payload: str) -> bool:
        ...
        # Environment signals
        if (
            "xss_stored_posts_success" in response.text
            or "xss_stored_comments_success" in response.text
        ):
            return True
    ...
    def _validate_idor(self, response) -> bool:
        ...
        # Environment signals
        if (
            "idor_profile_view_success" in response.text
            or "idor_orders_view_success" in response.text
        ):
            return True
