    ...
    def train(self, total_episodes=1000):
        ...
        try:
            for episode in range(self.start_episode, total_episodes + 1):
                ...
                if episode % 10 == 0:
                    print(
                        f"Episode {episode}: Target={target['name']} | Reward={reward:.1f} | Vulns={vulns}"
                    )
            ...
            print(
                f"✅ Checkpoint saved: checkpoints/{self.checkpoint_prefix}_ep{current_episode}.pth"
            )
