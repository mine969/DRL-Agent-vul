...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size: int) -> Tuple[
        ...
        actions = torch.tensor([e.action for e in experiences], dtype=torch.long).to(
            self.device
        )
        rewards = torch.tensor([e.reward for e in experiences], dtype=torch.float32).to(
            self.device
        )
        ...
        return states, actions, rewards, next_states, dones, importance_weights, indices
    ...
    def __init__(
        ...
        if use_noisy:
            self.value_stream = nn.Sequential(
                NoisyLinear(feature_output_size, 128), nn.ReLU(), NoisyLinear(128, 1)
            )
        else:
            self.value_stream = nn.Sequential(
...
class ImprovedDQNAgent:
    ...
    def replay(self) -> Optional[float]:
        ...
        loss = (
            self.loss_function(current_q_values.squeeze(1), target_q_values)
            * importance_weights
        ).mean()
