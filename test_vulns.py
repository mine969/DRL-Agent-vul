...
def test_vulnerability(name, url, method='GET', data=None, params=None, headers=None, expected_header='X-Vuln-Confirmed', session=None):
    print(f"\n📍 Testing {name} ({url})")
    ...
    try:
        ...
        if expected_header in r.headers:
            print(f"  ✅ SUCCESS: {expected_header} found! ({r.headers[expected_header]})")
            return True
        else:
...
def get_session(login_url, username='admin', password='admin123'):
    ...
    try:
        r = s.post(login_url, data={'username': username, 'password': password}, timeout=5)
...
def run_all_tests():
    ...
    fs_session = get_session("http://localhost:5006/login", username='user', password='password')
    if fs_session:
        results.append(test_vulnerability(
