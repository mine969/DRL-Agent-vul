...
class TrainingConfig:
    ...
    max_episodes: int = 3000  # active budget as of the ablation study (10k run archived, see checkpoints/archive_10k_run/)
...
class ScanConfig:
    ...
    crawl_depth: int = 100  # bumped from 30: mock targets have templated routes
    # (per-product/per-profile URLs) that can exceed 30 discovered pages
    max_urls: int = 1000
    intensity: int = 50  # bumped from 3: no hard cap enforced in code despite the
    # old "1-5 scale" comment; higher intensity gives Noisy Net exploration
    # (see agent/improved_dqn_agent.py act()) more attempts per page to find vulns
