...
from flask import Flask, request, jsonify, session, send_from_directory, render_template_string, redirect, url_for, make_response
import sqlite3
import hashlib
import os
import uuid
...
def register():
    ...
    try:
        ...
        error_response = make_response(render_template_string(
            HTML_TEMPLATE.replace('{{ content | safe }}', f'<div class="alert">Error: {str(e)}</div>')
        ))
...
def api_search():
    ...
    try:
        ...
        sql_query = f"SELECT id, username FROM users WHERE username LIKE '%{query}%' LIMIT 10"
