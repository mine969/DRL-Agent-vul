...
def benchmark():
    ...
    try:
        ...
        env = WebSecurityGym(
            target_url="http://localhost:5002", mode="mock_targets", verbose=False
        )
        ...
        agent = ImprovedDQNAgent(
            state_dim, action_dim, use_prioritized_replay=True, seed=42
        )
        ...
        for i in range(agent.batch_size + 10):
            ...
            state = (
                next_state if not done else np.random.rand(state_dim).astype(np.float32)
            )
